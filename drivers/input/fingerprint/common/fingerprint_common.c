...
static int pinctrl_select_pin(struct pinctrl *p,char *name)
{
	...
	{
		pr_err(KERN_ERR"Can not find state %s\n",name);
		goto exit;
	}

	return ret;
exit:
	pr_err(KERN_ERR"Can not select state %s\n",name);
	return ret;
}
#endif
...
int commonfp_power_on()
{
	...
	pr_debug(KERN_INFO"power on OK!!!");
	return ret;
}
...
int commonfp_power_off()
{
	...
	pr_debug(KERN_INFO"power off OK 111 !!!");
	return 0;
}
...
int commonfp_hw_reset(int ms)
{
	...
	pr_debug(KERN_INFO"hw reset success,ret:%d\n",ret);
	return ret;
exit:
	pr_err(KERN_ERR"fingerprint_hw_reset failed,ret:%d\n",ret);
	return ret;
}
	    ...
	    const char *name, void *dev)
{
	...
	{
		ret = 0;
		pr_debug(KERN_INFO"irq has been requested\n");
		return ret;
	}
...
void commonfp_free_irq(void *dev_id)
{
	if(irq_flag == 0)
	{
		pr_debug(KERN_INFO"irq has been free\n");
		return;
	}
	free_irq(fp_g.irq_num,dev_id);
	irq_flag = 0;
	pr_debug(KERN_INFO"commonfp free_irq SUCCESS\n");
...
{	
	...
	pr_debug(KERN_INFO"fp_common_probe start;");

#if USE_COMMON_PINCTRL
	fp_g.fp_pinctrl = devm_pinctrl_get(&fp_g.pdev->dev);
	if(!fp_g.fp_pinctrl)
	{
		pr_err(KERN_ERR"Failed to get common fp pinctrl\n");
		return -1;
	}
	...
	for(i = 0; i < ARRAY_SIZE(fp_g.states); i++)
	{
		...
		if (IS_ERR(state)) {
			pr_err(KERN_ERR"commonfp:cannot find '%s'\n", pinctrl_names[i]);
			ret = -EINVAL;
			return ret;
		}
		...
		if(ret){
			...
			pr_debug(KERN_ERR"select state %s OK !!!\n",pinctrl_names[i]);
		}
	}
	...
	if(ret){
		pr_err(KERN_ERR"can not select commonfp_irq_active state \n");
	}else{
		pr_debug(KERN_ERR"select commonfp_irq_active state  OK !!!\n");
	}

	fp_g.pwr_gpio = of_get_named_gpio(dev->of_node, "common,gpio_vdd", 0);
	ret = gpio_request(fp_g.pwr_gpio, "fingerprint_common_power");
	if(ret) {
		pr_err(KERN_ERR"Failed to request pm660l  GPIO(5). err= %d\n",ret);
		return -1;
	}
	...
	{
		pr_err(KERN_ERR"commonfp:get reset gpio is invaild!\n");
		return -1;
	}

	if (!gpio_is_valid(fp_g.rst_gpio))
	{
		pr_err(KERN_ERR"commonfp:get irq gpio is invaild!\n");
		return -1;
	}

	ret = devm_gpio_request(dev, fp_g.irq_gpio, "commonfp,gpio_irq");
	if (ret) {
		pr_err(KERN_ERR"failed to request irq gpio %d\n", fp_g.irq_gpio);
		return -1;
	}

	ret = devm_gpio_request(dev, fp_g.rst_gpio, "commonfp,gpio_rst");
	if (ret) {
		pr_err(KERN_ERR"failed to request rst gpio %d\n", fp_g.rst_gpio);
		goto error;
	}
	...
	pr_info(KERN_ERR"fp_common_probe probe OK\n");
...
static int __init fingerprint_resource_init(void)
{
	...
	if(ret){
		pr_err(KERN_ERR"commonfp:register common fingerprint platform driver failed:%d\n",ret);
		return ret;
	}
	pr_info(KERN_INFO"commonfp fingerpint init OK!!!,ret = %d\n",ret);
