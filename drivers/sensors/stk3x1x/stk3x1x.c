...
static void stk3x1x_proc_plat_data(struct stk3x1x_data *ps_data, struct stk3x1x_platform_data *plat_data)
{
	...
	{
		pr_debug("%s: wait_reg should be larger than 2, force to write 2\n", __func__);
		ps_data->wait_reg = 2;
	}
	else if (ps_data->wait_reg > 0xFF)
	{
		pr_debug("%s: wait_reg should be less than 0xFF, force to write 0xFF\n", __func__);
...
#ifdef STK_DEBUG_PRINTF
		pr_debug("%s: als=%d, ir=%d, als_correct_factor=%d", __func__,
...
static int stk3x1x_setup_irq(struct i2c_client *client)
{
	...
	if (err < 0)
	{
		pr_debug("%s: request_any_context_irq(%d) failed for (%d)\n", __func__, irq, err);
...
static int stk3x1x_suspend(struct device *dev)
{
	...
	if(ps_data->ps_enabled)
	{
		...
		if(device_may_wakeup(&client->dev))
		{
			...
			if (err)
				pr_debug("%s: set_irq_wake(%d) failed, err=(%d)\n", __func__, ps_data->irq, err);
		}
		else
		{
...
static int stk3x1x_resume(struct device *dev)
{
	...
	if(ps_data->ps_enabled)
	{
		...
		if(device_may_wakeup(&client->dev))
		{
			...
			if (err)
				pr_debug("%s: disable_irq_wake(%d) failed, err=(%d)\n", __func__, ps_data->irq, err);
		}
#endif
	}
#ifndef SPREADTRUM_PLATFORM
